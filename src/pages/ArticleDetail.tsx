...
const ArticleDetail = () => {
  ...
  // Determine media to show: prefer media_url (could be video) then image
  const mediaCandidates = [
    // legacy mapping variety
    // @ts-ignore
    article.media_url,
    // @ts-ignore
    (article as any).media,
    article.image,
    // @ts-ignore
    (article as any).image_url,
  ].filter(Boolean) as string[];

  const mediaUrl = mediaCandidates.length > 0 ? mediaCandidates[0] : '';
          ...
          <header className="mb-8">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Button variant="ghost" size="sm" onClick={() => handleShare('facebook')} className="p-2"><Facebook className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => handleShare('twitter')} className="p-2"><Twitter className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => handleShare('linkedin')} className="p-2"><Linkedin className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => handleShare('copy')} className="p-2"><Share2 className="w-4 h-4" /></Button>
              </div>
            </div>
          </header>

          {/* Featured Media */}
          <div className="mb-8 rounded-2xl overflow-hidden shadow-elegant bg-black">
            {mediaIsVideo ? (
              <video
                src={mediaUrl}
                controls
              ...
              <img src={mediaUrl} alt={article.title} className="w-full h-64 md:h-96 object-cover" />
            ) : (
              <div className="w-full h-64 md:h-96 flex items-center justify-center text-muted-foreground">Aucun visuel</div>
            )}
          </div>
