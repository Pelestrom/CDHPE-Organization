...
class ApiService {
  ...
  async getPublications(limit?: number, offset?: number) {
    try {
      let query = (supabase as any)
        .from('publications')
        .select('*, categories:categorie_id(nom), teams:equipe_id(nom)')
        .eq('published', true)
  ...
  async getEvents(status?: 'upcoming' | 'past') {
    try {
      let query = (supabase as any)
        .from('events')
        .select('*, event_types:type_event_id(nom)')
  ...
  async adminGetPublications() {
    const { data, error } = await (supabase as any)
      .from('publications')
      .select('*, categories(nom), teams(nom)')
      .order('created_at', { ascending: false });
    if (error) throw error;

    return data.map((pub: any) => ({
      ...pub,
      categorie_nom: pub.categories?.nom,
      equipe_nom: pub.teams?.nom,
    }));
  }
  ...
  async adminCreatePublication(publication: Partial<any>) {
    const payload: any = {
      ...
      type: publication.type_media_principal === 'texte' ? 'text' : 
            publication.type_media_principal === 'image' ? 'photo' : 
            publication.type_media_principal,
      category: 'Actualités', // Valeur par défaut pour respecter la contrainte NOT NULL
    ...
    try {
      ...
      console.error('adminCreatePublication error:', err);
      throw err;
    }
  }
  ...
  async adminUpdatePublication(id: string, updates: Partial<any>) {
    ...
    // Gardez la catégorie existante ou mettez une valeur par défaut
    updateData.category = 'Actualités'; // Respecte la contrainte NOT NULL
  ...
  async adminCreateEvent(event: Partial<any>) {
    const payload: any = {
      ...
      time: event.time || event.heure || '00:00:00',
